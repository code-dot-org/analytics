{%- macro drop_vestigial_relations(
  ...
  {%- if exclude_schemas | is_string %}
    {%- set exclude_schemas = [exclude_schemas] %}
  {%- elif exclude_schemas | length == 0 %}
  ...
  {%- if execute %}
    {%- set manifest = adapter.get_manifest() %}
    {%- set nodes_to_check =
      manifest.nodes.values() | selectattr('resource_type', 'in', ['model', 'seed']) | list %}
    {%- set graph_schemas = [] %}
    ...
    {#- Check if there are any vestigial tables -#}
    {%- if full_results is none or full_results | length == 0 %}
    ...
    {%- else %}
      ...
      {#- Check if there are tables to delete and build the drop statements -#}
      {%- if to_delete %}
        {%- set drop_query_list = ['begin;'] %}
        {%- for item in to_delete %}
          {%- set drop_statement %}  
          drop {{ item[2] }} if exists "{{ item[0] }}"."{{ item[1] }}" cascade;
          {%- endset %}
        ...
        {#- Execute or log the drop statements based on dry_run flag -#}
        {%- if not dry_run %}
